...
function mockCalculate(data) {
  ...
  // Compute two segments: current -> pickup and pickup -> dropoff, then sum.
  function normalize(s) {
    return (s || "").toLowerCase().replace(/[^a-z,]/g, "");
  }

  function deterministicDistance(from, to) {
    ...
    // map to range 50..1200 miles
    return 50 + (abs % 1151);
  }

  const seg1 = deterministicDistance(data.current_location || "", data.pickup_location || "");
  const seg2 = deterministicDistance(data.pickup_location || "", data.dropoff_location || "");
  const total_distance = Math.min(5000, Math.max(10, Math.round(seg1 + seg2)));
  const averageSpeedMph = 60; // average driving speed
  const estimated_drive_time = Math.round(total_distance / averageSpeedMph); // hours
